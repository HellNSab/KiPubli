import type { BookMetadata } from "./booksApi";
...
async function putJsonFile(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      (err as { message?: string }).message ?? `GitHub API ${res.status}`,
    );
  }
}

export async function appendIsbnCache(
  isbn: string,
  book: BookMetadata,
): Promise<void> {
  if (!hasToken()) return;
  const { sha, data } =
    await getJsonFile<Record<string, BookMetadata>>(ISBN_CACHE_PATH);
  if (data[isbn]) return;
  data[isbn] = book;
  await putJsonFile(
    ISBN_CACHE_PATH,
