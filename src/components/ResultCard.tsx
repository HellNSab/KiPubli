...
function RoleBox({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-stone-200 bg-white px-4 py-3.5 dark:border-stone-800 dark:bg-dark-card">
      <p className="text-sm leading-relaxed text-muted dark:text-subtle">
        {children}
      </p>
    </div>
  );
...
function EditionTab({ chain }: { chain: OwnershipChain }) {
  const ownerSubtitle = chain.group.listed
    ? "Groupe coté en bourse"
    : "Groupe familial, non coté";
...
function DiffusionTab({ chain }: { chain: OwnershipChain }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <RoleBox>
        Le diffuseur envoie des représentants en librairie pour présenter les
        nouveautés, négocier les mises en avant et transmettre les commandes.
      </RoleBox>
      ...
      {isExternal && (
        <WarningBox>
          Attention : malgré son indépendance éditoriale, cet éditeur diffuse
          via {diffuseur_owner}. Une partie des flux financiers transite par ce
          groupe.
        </WarningBox>
      )}
    </div>
...
function DistributionTab({ chain }: { chain: OwnershipChain }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <RoleBox>
        Le distributeur achemine physiquement les livres vers les librairies,
        gère les stocks en entrepôt et traite les retours des invendus.
      </RoleBox>
      ...
      {isExternal && (
        <WarningBox>
          Attention : malgré son indépendance éditoriale, cet éditeur distribue
          via {distributeur_owner}. Une partie des flux financiers transite par
          ce groupe.
        </WarningBox>
      )}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-start gap-4 rounded-2xl border border-stone-200 bg-white p-4 dark:border-stone-800 dark:bg-dark-card">
        ...
        <div className="min-w-0">
          ...
          {badge && (
            <span
              className={`mt-2 inline-block rounded-lg px-2.5 py-1 text-xs font-semibold ${badge.className}`}
            >
              {badge.label}
            </span>
          )}
