  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (msg === "TOKEN_MISSING") {
        setErrorMsg(
          "Fonctionnalité non configurée. Veuillez contacter l'administrateur.",
        );
      } else {
        setErrorMsg(
        ...
        {status === "success" ? (
          <div className="mt-6 flex flex-col items-center gap-4 py-4 text-center">
            ...
            <p className="text-sm text-stone-500">
              Nous prendrons en compte votre retour lors de la prochaine mise à
              jour des données.
            </p>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="mt-5 flex flex-col gap-4">
            ...
            <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm text-stone-700 dark:bg-dark-card dark:text-subtle">
              <p>
                <span className="font-medium">Titre :</span> {book.title}
              </p>
              <p className="mt-1">
              ...
              {chain ? (
                <p className="mt-1">
                  ...
                  {chain.publisher.name} → {chain.group.name} →{" "}
                  {chain.group.owner}
                </p>
              ) : (
                <p className="mt-1 text-amber-700">
                  Non identifié dans la base
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              >
                Commentaire{" "}
                <span className="font-normal text-stone-400 dark:text-subtle">
                  (facultatif)
                </span>
              </label>
              <textarea
